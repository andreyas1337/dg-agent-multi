...
> measurably faster (~3–5× in our tests).
...
### An agent is just an `UpdateThink` + `UpdateSpeak` payload

A persona is fully described by its think config (provider, model, prompt,
functions) and its voice. So each agent can run its **own model — even its own
provider**: a cheap, fast router can hand off to a stronger specialist model
mid-call.
...
### Conversation history lives server-side — no summarizer

Because there is one continuous session, you **do not pass context yourself**.
`UpdateThink` replaces the *configuration*, not the conversation. The new agent
sees the prior turns automatically.
...
When the LLM calls your `transfer_to_agent` tool, do **not** answer it first.
Instead:

1. Send `UpdateThink` for the target → wait for `ThinkUpdated`.
2. If the target declares its own voice, send `UpdateSpeak` → wait for `SpeakUpdated`.
...
### Distinct vs. seamless is emergent — no mode flag
...
- **Voice** — set an agent's `voice` and the caller hears a distinct specialist;
  **omit** it and the agent inherits the current voice, so the caller keeps
  hearing one person.
...
billing = Agent(                       # voice omitted -> seamless, same voice
    name="billing",
...
Say *"what's my balance?"* — the assistant silently gains billing tools and
answers in the **same** voice (seamless). Then say *"my laptop won't turn on"* —
...
| Handoff latency (measured*) | **~948 ms** US / ~215 ms EU **+ summarizer + audio re-stream** | **~300 ms** US / ~47 ms EU |
...
reconnect figure is connection + `Settings` → `SettingsApplied` only; it does
...
main.py           FastAPI proxy: browser ⇄ Deepgram, feeds events to the orchestrator
